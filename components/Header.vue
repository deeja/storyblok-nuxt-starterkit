    ...
    <nav class="md:mx-auto md:w-2/3 md:my-2 p-2 md:p-0">
      <div class="flex">
        ...
        <nuxt-link :to="`/${blok.logo_link.cached_url}`" v-if="blok.logo" class="mr-3 mt-1">
         ...
         <div v-if="blok.nav_links && blok.nav_links.length" class="hidden md:flex">
        <component
          :key="blok._uid"
      ...
      <ul v-if="blok.second_nav_links && blok.second_nav_links.length" class=" hidden md:flex">
        <component
          :key="blok._uid"
      ...
      <ul v-if="showItemsMobile && blok.nav_links && blok.nav_links.length" class="flex-column md:hidden">
        <component
          :key="blok._uid"
          class="p-2 hover:text-blue-400"
          v-for="blok in blok.nav_links"
